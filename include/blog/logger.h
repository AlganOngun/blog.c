...
char *generate_status_text(BLOG_LOG_TYPE type) {
  ...
  switch (type) {
  case INFO:
    status_text = "\033[1;32mINFO\033[0m";
    break;
  case WARNING:
    status_text = "\033[1;33mWARNING\033[0m";
    break;
  case DEBUG:
    status_text = "\033[0;34mDEBUG\033[0m";
    break;
  case ERROR:
...
char *generate_status_colored_text(const char *text, BLOG_LOG_TYPE type) {
  char *colored_text =
      (char *)malloc(strlen(text) + COLOR_CODES_SIZE_PLUS_NULL);

  switch (type) {
  case INFO:
    snprintf(colored_text, strlen(text) + COLOR_CODES_SIZE_PLUS_NULL, "%s%s%s",
...
void blog_log(const char *log, BLOG_LOG_TYPE log_type,
              const char *calling_file_name, const char *calling_function_name,
  ...
  char *colored_formatted_log =
      generate_status_colored_text(formatted_log, log_type);

  char log_template[LOG_TEMPLATE_SIZE];
  snprintf(log_template, sizeof(log_template),
           "[%s] [%02d/%02d/%d] [%02d:%02d:%02d] %s from %s():%d -> ",
           calling_file_name, time.tm_mday, time.tm_mon + 1,
           time.tm_year + 1900, time.tm_hour, time.tm_min, time.tm_sec,
           generate_status_text(log_type), calling_function_name,
           calling_line_number);

  char final_log[strlen(log_template) + strlen(colored_formatted_log) + 1];

  snprintf(final_log, sizeof(final_log), "%s%s", log_template,
           colored_formatted_log);
